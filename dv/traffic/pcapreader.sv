...
(
    ...
    avalon_if.out       from_reader_avalon,  // Avalon-ST interface output
    taxi_axis_if.src    from_reader_axis,    // AXI4-Stream interface output
    ...
    always @(posedge clk)
    begin
        if (from_reader_avalon.ready == 1'b1 || from_reader_axis.tready == 1'b1) begin
            if (eof != 0 || pcapfinished == 1) begin
    	        ...
    	        if (SIGNAL_TYPE == "avalon") begin
		            ...
		            from_reader_axis.tlast <= 0;
                    from_reader_axis.tvalid <= 0;
	                from_reader_axis.tdata <= {DATA_WIDTH{1'b0}};
	            end
            end else if (eof == 0 && actLen == 0 && rst == 0) begin
    	        // read packet header
    	        ...
    	        if (r != 0) begin
                    ...
                    if (SIGNAL_TYPE == "avalon") begin
                        ...
                        from_reader_axis.tvalid <= 0;
                        from_reader_axis.tdata <= {DATA_WIDTH{1'b0}};
                        from_reader_axis.tkeep <= {KEEP_WIDTH{1'b0}};
                    end
	            end else begin
                    pcapfinished <= 1;
                    if (SIGNAL_TYPE == "avalon") begin
    	    ...
    	    end else if (actLen > 0 && rst == 0) begin
                if (toNanos == 1) begin
                ...
                if (cnt_clk < timestamp_new_ps) begin  // if current value of clock counter is not larger than precalculated timestamp, don't start with new packet
    	            ...
    	            if (from_reader_avalon.ready == 1'b1 || from_reader_axis.tready == 1'b1) begin  // check if ready signal is active; if ready is not active, there is backpressure
                        newpkt <= 0;
                        ...
                        if (SIGNAL_TYPE == "avalon") begin  // check signal type and assign values to signals on the appropriate interface
                            ...
                            from_reader_axis.tlast <= actLen <= bytes_data_out;
                        end
                        ...
                        if (capLen == actLen) begin  // case 1: captured and actual length (on wire) are equal; assign values for data and empty/keep signals
                            for (i=0; i < bytes_data_out; i = i + 1) begin
                                if (SIGNAL_TYPE == "avalon") begin
                                    ...
                                    from_reader_axis.tdata[i*8+:8] <= capLen > i ? $fgetc(file) : 8'b0;
                                end
                            end
                            ...
                            if (SIGNAL_TYPE == "avalon") begin
                                ...
                                if (capLen > bytes_data_out) begin
                                    from_reader_axis.tkeep <= {KEEP_WIDTH{1'b1}};
                                end else begin
                                    for (i = 0; i < KEEP_WIDTH; i = i + 1) begin
                                        if (i < KEEP_WIDTH - (bytes_data_out - capLen)) begin
                                            from_reader_axis.tkeep[i] <= 1'b1;
                                        end else begin
                                            from_reader_axis.tkeep[i] <= 1'b0;
                                        end
                                    end
                                end
                            end
                            ...
                            if (capLen > 0) begin  // capLen > 0 means that there is still some real content in pcap file, so we can access it through "fgetc(file)"...
                                ...
                                for (i = 0; i < bytes_data_out; i = i + 1) begin
                                    if (SIGNAL_TYPE == "avalon") begin
                                        ...
                                        from_reader_axis.tdata[i*8+:8] <= 8'b0;
                                    end
                                end
        	        ...
        	        if (eof == 0 && capLen != 1) begin
                        if (SIGNAL_TYPE == "avalon") begin
		                    from_reader_avalon.valid <= 1'b1;
		                end else begin
                            from_reader_axis.tvalid <= 1'b1;
	            	    end
		            end
	            end else begin
	                if (SIGNAL_TYPE == "avalon") begin
		                from_reader_avalon.eop <= 1'b0;
		                from_reader_avalon.sop <= 1'b0;
		                from_reader_avalon.valid <= 1'b0;
                        from_reader_avalon.data <= {DATA_WIDTH{1'b0}};
                        from_reader_avalon.empty <= {EMPTY_WIDTH{1'b0}};
                    end else begin
                        from_reader_axis.tlast <= 1'b0;
		                from_reader_axis.tvalid <= 1'b0;
                        from_reader_axis.tdata <= {DATA_WIDTH{1'b0}};
                        from_reader_axis.tkeep <= {KEEP_WIDTH{1'b0}};
		            end
	            end
    ...
    assign from_reader_axis.tid = '0;
    assign from_reader_axis.tdest = '0;
    assign from_reader_axis.tuser = '0;
endmodule
