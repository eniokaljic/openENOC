...
The endpoint HAL is based on a memory-mapped CSR architecture that exposes endpoint configuration and operational state to software. Through this interface, software can configure endpoint behavior, monitor operational status, and control communication-related functionality.
