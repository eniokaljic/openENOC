...
```bash
make APP=csr_smoke EP=openenoc
```

The selected endpoint provides the generated CSR header at:

```text
build/hal/<endpoint>/include/csr.h
```
...
| `IMEM_DEPTH` | `8192` | Maximum number of 32-bit words in the IMEM image |
...
For example, `APP=csr_smoke EP=openenoc` generates:
...
`IMEM_DEPTH`. Because this file has an endpoint-scoped name, building another
application for the same endpoint replaces `imem.mem` with the newly selected
firmware image.

To request the IMEM file explicitly, run:

```bash
make APP=csr_smoke EP=openenoc imem-init
```
